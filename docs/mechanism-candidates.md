...
| 5 | 长会话要主动 compact + 写 handoff | Skill / 提示词 | A | `registerMessageBuilder` plugin（双产物） | 候选（暂缓）— 触发条件：见 [ADR-004](decisions/ADR-004-p5-spike-pause.md)（CLI 载体稳定性恢复 + 实验环境与生产环境对齐）。#6 hook 已实证触发，结论保留。**[ADR-002 Update 4](decisions/ADR-002-project-shape.md) 修正：VS Code 扩展代码层有 `registerMessageBuilder` 注册接口（Update 3 确认），但 plugin 安装路径为 `.cline/plugins/`（非 `.cline/<pluginName>/`），manifest 为 `package.json` 含 `cline` 字段（非 `managed.json`）。官方文档明确 VS Code 扩展"不支持 plugin"但代码层有完整注册系统，手动放文件到 `.cline/plugins/` 能否触发自动发现待 Capability Probe 5 实测。母本：[custom-compaction.ts](https://github.com/cline/cline/blob/main/sdk/examples/plugins/custom-compaction.ts)** |
