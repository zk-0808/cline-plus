...
| 4 | 重复循环要主动跳出 | 提示词 / Skill | A | loop-guard plugin（基于工具调用相似度） | **候选（设计调整）** — 原方案 beforeModel 注入受 [codec bug §O8](decisions/investigation-note-cli-codec-content-map-bug.md) 阻塞（Verified：注入 content 为 string，codec `.map()` 必崩）。2026-07-01 采纳 V6 替代实现：afterTool 检测 + registerRule 动态注入（绕过 codec 路径，延迟一个 turn）。检测层已实测通过（[handoff.md §4](handoff.md)），V6 实现待 A5 行动项推进 |
