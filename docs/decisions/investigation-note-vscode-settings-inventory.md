...
>
> **U 项生命周期**（按 [evidence-governance.md §19](../evidence-governance.md) 补登，2026-07-02）：U3/U5/U6/U8/U9/U10/U11/U12 为待实测项，属官方文档/版本号类，默认 30 天。声明日 2026-06-27，必须补证日 2026-07-27。**因 §1.15 不可抗力（VS Code 扩展 plugin 系统不可用），U3/U5/U6/U11 暂无法实测**，超期处置为"保留并延期至 §1.15 解除"。

1. ~~**U1**：Settings 按钮内部有哪些设置项？~~ ✅ 已解答（J 段，5 tab）
2. ~~**U2**：Customize marketplace 有哪些 plugin？分类如何？~~ ✅ 已解答（K 段）
3. **U3**：MCP Servers 按钮内部的操作选项？（K3 部分覆盖，独立按钮待实测）— ⏸ 受 §1.15 阻塞
4. ~~**U4**：History 按钮内部的操作选项？~~ ✅ 已解答（L 段 Observation L3：Fuzzy search + 7 种排序）
5. **U5**：Account 按钮内部的操作选项？— ⏸ 受 §1.15 阻塞
6. **U6**：New Task 对话界面的 slash commands / 模式切换 / 上下文管理？— ⏸ 受 §1.15 阻塞
7. ~~**U7**：`cron.db` + `cron/` 目录对应的定时任务功能如何使用？~~ ✅ 已解答（L 段 L2：`cline schedule` wizard，但**仅 SDK/CLI/Kanban，VSCode 扩展不可用**）
8. **U8**：`teams.db` 对应的团队功能如何使用？— 需源码/官方文档核查
9. **U9**：`hub-daemon.log` 对应的 Hub daemon 是什么？（L 段 L2 部分解答：scheduling 依赖 hub，创建 schedule 时自动启动）— 需源码核查
10. **U10**：`ext-cline-pass` feature flag 启用了什么功能？— 需源码/服务端核查
11. **U11**：workspace 级 `.cline/plugins/` 自动扫描是否触发？（Probe 5 延续）— ⏸ 受 §1.15 阻塞
12. **U12（新）**：messageBuilder 扩展点 vs `before_agent_start` hook stage——哪个是 compact 流程的正确介入点？（P5 Spike 重启前提，需源码核查）— 需源码核查

| U# | 待实测项 | 声明日 | 必须补证日 | 补证路径 | 超期处置 |
|----|---------|--------|-----------|---------|---------|
| U3 | MCP Servers 按钮内部 | 2026-06-27 | 2026-07-27 | 用户实测 webview | 保留并延期至 §1.15 解除 |
| U5 | Account 按钮内部 | 2026-06-27 | 2026-07-27 | 用户实测 webview | 保留并延期至 §1.15 解除 |
| U6 | New Task 对话界面 | 2026-06-27 | 2026-07-27 | 用户实测 webview | 保留并延期至 §1.15 解除 |
| U8 | teams.db 团队功能 | 2026-06-27 | 2026-07-27 | 源码/官方文档核查 | 降级为 Unknown |
| U9 | hub-daemon.log | 2026-06-27 | 2026-07-27 | 源码核查 | 降级为 Unknown |
| U10 | ext-cline-pass flag | 2026-06-27 | 2026-07-27 | 源码/服务端核查 | 降级为 Unknown |
| U11 | workspace .cline/plugins/ 扫描 | 2026-06-27 | 2026-07-27 | 用户实测 | 保留并延期至 §1.15 解除 |
| U12 | messageBuilder vs before_agent_start | 2026-06-27 | 2026-07-27 | 源码核查 | 降级为 Unknown |
